...
m214<-multi.cc.pool.SE(s,r,m=4,simdata)
m216<-multi.cc.pool.SE(s,r,m=6,simdata)
m2110<-multi.cc.pool.SE(s,r,m=10,simdata)


beta1est<-c(ucox$estimate[1],uclogit$estimate[1],p2clogit$estimate[1],m212$estimate[1],
            m214$estimate[1],m216$estimate[1],m2110$estimate[1])
beta2est<-c(ucox$estimate[2],uclogit$estimate[2],p2clogit$estimate[2],m212$estimate[2],
            m214$estimate[2],m216$estimate[2],m2110$estimate[2])
SEbeta1est<-c(ucox$SE[1],uclogit$SE[1],p2clogit$SE[1],m212$SE[1],
              m214$SE[1],m216$SE[1],m2110$SE[1])
SEbeta2est<-c(ucox$SE[2],uclogit$SE[2],p2clogit$SE[2],m212$SE[2],
              m214$SE[2],m216$SE[2],m2110$SE[2])

dfbeta1<-data.frame(b1=beta1est, b1SE=SEbeta1est, x=rep(1:7))
dfbeta2<-data.frame(b2=beta2est, b2SE=SEbeta2est, x=rep(1:7))

# 
### using the plot in R base
set.seed(3433)
par(mar=c(5,5,3,3))
Text<-c("Cox","P0-1:1M","P2-1:1M","P2-1:2M","P2-1:4M","P2-1:6M","P2-1:10M")
plot(dfbeta1$x,dfbeta1$b1,
     ylim = c(0.001,0.008), xlim = c(0.5,7.8),
...
model3<-multi.cc.pool.SE(s,r,m=4,simdata)
model4<-multi.cc.pool.SE(s,r,m=6,simdata)
model5<-multi.cc.pool.SE(s,r,m=10,simdata)
...
set.seed(3433)
par(mar=c(5,5,3,3))
Text<-c("Cox","P0-1:1M","P4-1:1M","P4-1:2M","P4-1:4M","P4-1:6M","P4-1:10M")
plot(dfbeta1$x,dfbeta1$b1,
     ylim = c(0.001,0.008), xlim = c(0.5,7.8),
     pch=19, xlab="Type of model", ylab='Hazard Ratio',
     main='Hazard Ratio of Creatinine by model'
...
est.mat.b0<-data.frame(ests.mat.uncox$`Beta0-cox`,ests.mat.p0$`Beta0-P0`,
                       #ests.mat.p2$`Beta0-P2`,ests.mat.m2$`Beta0-m2`,
                       ests.mat.m4$`Beta0-m4`,ests.mat.m6$`Beta0-m6`)
names(est.mat.b0)<-c('cox-b1','pool0-b1',
                     #'Pl6-1:1','P6-1:2',
                     'P6-1:4','P6-1:6')
boxplot(est.mat.b0,main='Hazard Ratio of X by model',ylab ="Hazard Ratio",
...
est.mat.b1<-data.frame(ests.mat.uncox$`Beta1-cox`,ests.mat.p0$`Beta1-P0`,
                       ...
                       ests.mat.m6$`Beta1-m6`)
names(est.mat.b1)<-c('cox-b2','pool0-b2',
                     # 'P6-1:1','P6-1:2',
                     'P6-1:4','P6-1:6')
boxplot(est.mat.b1,main='Hazard Ratio of Z by model',
        ylab ="Hazard Ratio", xlab ="Type of model",
...
set.seed(3433)
par(mar=c(5,5,3,3))
Text<-c("Cox","P0-1:1M","P6-1:1M","P6-1:2M","P6-1:4M","P6-1:6M","P6-1:10M")
plot(dfbeta1$x,dfbeta1$b1,
     ylim = c(0.001,0.008), xlim = c(0.5,7.8),
     pch=19, xlab="Type of model", ylab='Hazard Ratio',
     main='Hazard Ratio of Creatinine by model'
...
boxplot(est.mat.b0,main='Hazard Ratio of X in 1-1 matched sets',ylab ="Hazard Ratio",
...
boxplot(est.mat.b1,main='Hazard Ratio of Z in 1-1 matched sets',
        ylab ="Hazard Ratio", xlab ="Type of model",
...
######### lets try the same for the artifical data   ######################
...
set.seed(3433)
par(mar=c(5,5,3,3))
Text<-c("Cox","P0-1:1M","P2-1:1M","P4-1:1M","P6-1:1M")
plot(dfbeta1$x,dfbeta1$b1,
...
boxplot(est.mat.b0,main='Hazard Ratio of X in 1-10 matched sets',ylab ="Hazard Ratio",
        xlab ="Type of model", names = c('Cox','Pool0','Pool2','Pool4',
                                         'Pool6'
        ),
        col = "gray",ylim=c(-.2,4.0),las=1);
